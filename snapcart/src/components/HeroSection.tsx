  ...
  const slides = [
    {
      id: 1,
      icon: (
        <Leaf className="w-20 h-20 sm:w-28 sm:h-28 text-green-400 shadow-lg" />
      ),
      title: "Fresh Organic Groceries 🥦",
      subtitle:
        "Farm-Fresh Fruits,Vegetables, and daily essentials delivered to you.",
      btnText: "Shop Now",
      bg: "https://images.unsplash.com/photo-1688964420317-0c48f1d3781d?q=80&w=1585&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    },
    {
      id: 2,
      icon: (
        <Truck className="w-20 h-20 sm:w-28 sm:h-28 text-yellow-400 shadow-lg" />
      ),
    ...
    },
    {
      id: 3,
      icon: (
        <Smartphone className="w-20 h-20 sm:w-28 sm:h-28 text-blue-400 shadow-lg" />
      ),
      title: "Shop Anytime Anywhere 📱",
      subtitle: "Easy and seamless online grocery shopping experience",
      btnText: "Get Started",
      bg: "https://images.unsplash.com/photo-1483378096604-1294ca36d0f3?q=80&w=1470&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    },
  ];
  ...
  return (
    <div className="relative w-[98%] mx-auto mt-32 h-[80vh] rounded-3xl overflow-hidden shadow-2xl">
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3">
          {slides.map((_,index)=>(
            <button key={index} className={`w-3 h-3 rounded-full transition-all ${index === current ? "bg-white w-5" : "bg-white/50"}`}/>
          ))}
        </div>
    </div>
  )
